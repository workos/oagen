...
The resolution needs to handle:
0. **Manifest match** — If a `smoke-manifest.json` is present (generated by the emitter's `generateManifest`), use it for deterministic resolution. This is the preferred tier.
