...
Two phases: one-time setup (`/add-language` — scaffold emitter, verify compat if preserving an existing SDK, smoke test) then ongoing spec updates (`oagen diff` → `oagen generate` → `oagen verify`). See `docs/architecture/workflows.md`.
...
- [Workflows](docs/architecture/workflows.md) — setup (`/add-language`) vs. ongoing (spec update pipeline), with loop diagrams
...
- `/oagen:add-language <language>` — end-to-end orchestrator for adding a new target language
