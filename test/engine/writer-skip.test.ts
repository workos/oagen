...
describe('writeFiles skipIfExists', () => {
  ...
  it('overwrites auto-generated test file when overwriteExisting is true', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'oagen-skip-'));
    const header = '# This file is auto-generated by oagen. Do not edit.';
    try {
      const testsDir = path.join(tmpDir, 'tests');
      await fs.mkdir(testsDir, { recursive: true });
      const filePath = path.join(testsDir, 'test_foo.py');
      await fs.writeFile(filePath, `${header}\n\nold test content`, 'utf-8');

      const result = await writeFiles(
        [{ path: 'tests/test_foo.py', content: `${header}\n\nnew test content`, overwriteExisting: true }],
        tmpDir,
        { language: 'python', header },
      );

      expect(result.written).toContain('tests/test_foo.py');
      const content = await fs.readFile(filePath, 'utf-8');
      expect(content).toContain('new test content');
    } finally {
      await fs.rm(tmpDir, { recursive: true });
    }
  });

  it('overwrites hand-written test file when overwriteExisting is true', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'oagen-skip-'));
    const header = '# This file is auto-generated by oagen. Do not edit.';
    try {
      const testsDir = path.join(tmpDir, 'tests');
      await fs.mkdir(testsDir, { recursive: true });
      const filePath = path.join(testsDir, 'test_foo.py');
      await fs.writeFile(filePath, 'hand-written test content', 'utf-8');

      const result = await writeFiles(
        [{ path: 'tests/test_foo.py', content: `${header}\n\nnew generated content`, overwriteExisting: true }],
        tmpDir,
        { language: 'python', header },
      );
  ...
  it('skips hand-written test file when overwriteExisting is not set', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'oagen-skip-'));
    const header = '# This file is auto-generated by oagen. Do not edit.';
    try {
      const testsDir = path.join(tmpDir, 'tests');
      ...
      expect(result.skipped).toContain('tests/test_foo.py');
      const content = await fs.readFile(filePath, 'utf-8');
      expect(content).toBe('hand-written test content');
    } finally {
      await fs.rm(tmpDir, { recursive: true });
    }
  });
