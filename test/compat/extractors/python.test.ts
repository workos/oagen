...
import { pythonExtractor, createPythonExtractor } from '../../../src/compat/extractors/python.js';

const fixturePath = resolve(import.meta.dirname, '../../fixtures/sample-sdk-python');

// Create an extractor with Pydantic BaseModel configured (as a consumer would)
const pydanticExtractor = createPythonExtractor({ modelBaseClasses: ['BaseModel'] });
...
describe('pythonExtractor', () => {
  ...
  it('produces deterministic output', async () => {
    const surface1 = await pydanticExtractor.extract(fixturePath);
    const surface2 = await pydanticExtractor.extract(fixturePath);
  ...
  it('default pythonExtractor hints have empty modelBaseClasses', () => {
    expect(pythonExtractor.hints.modelBaseClasses).toEqual([]);
  });
