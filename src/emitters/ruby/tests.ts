...
function generateTestFile(service: Service, ctx: EmitterContext): string {
  ...
  lines.push(
    `        @client = ${ctx.namespacePascal}::Client.new(api_key: "sk_test_xxx")`,
  );
  lines.push("      end");
  ...
  // Idempotency tests
  const createOp = service.operations.find(
    (o) => o.name === "create" && o.idempotent,
  );
  if (createOp) {
    lines.push("");
...
function generateCrudTest(
  ...
  if (statusCode === 204) {
    ...
    lines.push(
      `          .to_return(status: ${statusCode}, body: load_fixture("${fixtureName}.json"))`,
    );
  }
  ...
  if (op.paginated) {
    lines.push(
      `        assert_pattern { response => ${ctx.namespacePascal}::Internal::CursorPage }`,
    );
    ...
    if (modelName !== "Object") {
      lines.push(
        `        assert_pattern { response => ${ctx.namespacePascal}::Models::${modelName} }`,
      );
    } else {
...
function generateErrorTests(
  ...
  // 401 test
  const listUrl = `${baseUrl}/${stripLeadingSlash(op.path).replace(/\/\{[^}]+\}/, "")}`;
  const authOp =
    service.operations.find((o) => o.name === "list") || op;
  ...
  lines.push("");
  lines.push(
    `        assert_raises(${ctx.namespacePascal}::AuthenticationError) do`,
  );
  const authCallArgs = authOp.pathParams.length > 0 ? '("test_id")' : "";
  lines.push(
    `          @client.${resourceMethod}.${authOp.name}${authCallArgs}`,
  );
  lines.push("        end");
  lines.push("      end");
...
function generateRetryTests(
  ...
  lines.push(
    `          .then.to_return({ status: 200, body: load_fixture("${fixtureName}.json") })`,
  );
  lines.push("");
  lines.push(
    `        client = ${ctx.namespacePascal}::Client.new(api_key: "sk_test_xxx", max_retries: 2)`,
  );
  lines.push(`        response = client.${resourceMethod}.${op.name}`);
  lines.push("");
  lines.push(
    `        assert_requested :${op.httpMethod}, "${url}", times: 2`,
  );
  lines.push("      end");
...
function generateIdempotencyTests(
  op: Operation,
  service: Service,
  ctx: EmitterContext,
): string[] {
  ...
  lines.push(
    '          .with(headers: { "Idempotency-Key" => "my_key" })',
  );
  lines.push(
    `          .to_return(status: 201, body: load_fixture("${fixtureName}.json"))`,
  );
  lines.push("");
  lines.push(
    `        @client.${resourceMethod}.create({ name: "Test" }, idempotency_key: "my_key")`,
  );
  lines.push("");
  lines.push(
  ...
  lines.push(
    '          headers: { "Idempotency-Key" => "my_key" }',
  );
  lines.push("      end");
  lines.push("");
  ...
  lines.push(`        stub_request(:post, "${url}")`);
  lines.push(
    '          .with { |req| captured_key = req.headers["Idempotency-Key"]; true }',
  );
  lines.push(
    `          .to_return(status: 201, body: load_fixture("${fixtureName}.json"))`,
  );
  lines.push("");
  lines.push(
    `        @client.${resourceMethod}.create(name: "Test")`,
  );
  lines.push("");
  lines.push("        refute_nil captured_key");
  lines.push(
    '        assert_match(/\\A[0-9a-f-]{36}\\z/i, captured_key)',
  );
  lines.push("      end");
